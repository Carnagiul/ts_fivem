...
var require_RowDataPacket = __commonJS({
  "node_modules/mysql/lib/protocol/packets/RowDataPacket.js"(exports, module2) {
    ...
    function parse(parser, fieldPackets, typeCast2, nestTables, connection2) {
      var self2 = this;
      var next = function() {
    ...
    Connection.prototype.connect = function connect(options, callback) {
      ...
      if (!this._connectCalled) {
        ...
        this._protocol.on("data", function(data) {
          connection2._socket.write(data);
        });
        this._socket.on("data", wrapToDomain(connection2, function(data) {
          connection2._protocol.write(data);
        }));
    ...
    if (tls.TLSSocket) {
      Connection.prototype._startTLS = function _startTLS(onSecure) {
        ...
        createSecureContext(this.config, function(err, secureContext) {
          ...
          connection2._protocol.on("data", function(data) {
...
// server/Class/Identity.ts
var tableName = "identities";
var identities = [];
...
function loadIdentitiesAfterDbStart() {
  loadIdentities();
  setTimeout(setAsReady, 1e3);
}
function loadPlayersAfterDbStart() {
  loadPlayers();
  setTimeout(loadIdentitiesAfterDbStart, 1e3);
}
...
// server/Class/Player.ts
var tableName2 = "players";
var players = /* @__PURE__ */ new Map();
var playersBySource = /* @__PURE__ */ new Map();
var playerLoaded2 = false;
function loadPlayers() {
  connection.query("SELECT * FROM `" + tableName2 + "` WHERE `deleted_at` IS NULL OR `deleted_at` < NOW(); ", (err, results) => {
    if (err)
      console.log(err);
    else {
      results.forEach((x) => {
...
function savePlayer(steamID) {
  const date = new Date();
  var player = players.get(steamID);
  connection.query("UPDATE `" + tableName2 + "` SET `pos_x` = ?, `pos_y` = ?, `pos_z` = ?, `updated_at` = ? WHERE `steam_id` = ?", [player.pos_x, player.pos_y, player.pos_z, getDateString(), steamID], (err, results) => {
    if (err)
      console.log(err);
    else {
...
function createPlayer(steamID) {
  const date = new Date();
  connection.query("INSERT INTO `" + tableName2 + "` (`steam_id`, `pos_x`, `pos_y`, `pos_z`, `created_at`, `updated_at`) VALUES (?, ?, ?, ?, ?, ?)", [steamID, "0.0", "0.0", "0.0", getDateString(), getDateString()], (err, results) => {
    if (err)
      console.log(err);
    else {
      var player = {
